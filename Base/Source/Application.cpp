...
void Application::Run()
{
	...
	while (!glfwWindowShouldClose(m_window)&& !IsKeyPressed(VK_ESCAPE)) //add in menu state here )
	{
...
bool Application::GetKeyboardUpdate()
{
	...
	{
		scene->UpdateWeaponStatus(Scene3D::WA_RELOAD);
		bRButtonState = true;
	}
	if(!IsKeyPressed('R') && bRButtonState)
	{
	...
	{
		scene->UpdateWeaponStatus(Scene3D::WA_CHANGEWEAPON_1);
		b1ButtonState = true;
	}
	if(!IsKeyPressed('1') && b1ButtonState)
	{
	...
	{
		scene->UpdateWeaponStatus(Scene3D::WA_CHANGEWEAPON_2);
		b2ButtonState = true;
	}
	if(!IsKeyPressed('2') && b2ButtonState)
	{
...
bool Application::GetMouseUpdate()
{
	...
	//{
	//	scene->UpdateWeaponStatus(Scene3D::WA_FIRE);
	//}
