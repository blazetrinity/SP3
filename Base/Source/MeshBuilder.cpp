...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char> &heightMap)
{
	...
	for(unsigned z = 0; z < terrainSize; ++z)
	{
		for(unsigned x = 0; x < terrainSize; ++x)
		{
			...
			Vector3 Origin(x, (float)(heightMap[z * terrainSize + x]), z);

			Vector3 Forward, Right;

			if(x + 1 < terrainSize && z + 1 < terrainSize)
			{
				Forward.Set(x, (float)(heightMap[(z + 1) * terrainSize + x]), z + 1);
				Right.Set(x + 1, (float)(heightMap[z * terrainSize + x]), z);
				v.normal = ((Forward - Origin).Cross(Right - Origin)).Normalize();
			}
			else
			{
				v.normal.Set(0, 1, 0);
...
TileMap* MeshBuilder::GenerateTileMap(const std::string &meshName, unsigned numRow, unsigned numCol)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	TileMap *mesh = new TileMap(meshName);
